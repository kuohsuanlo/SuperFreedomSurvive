...
final public class spawner_block {
    ...
    static org.bukkit.metadata.FixedMetadataValue block_key = UseBlock.block_key("spawner_block"); // 創建特殊類型資料
    ...
    static final public ItemStack Get() {
        ...
        meta.getCustomTagContainer().setCustomTag(item_key, ItemTagType.STRING, "spawner_block"); // 登入key

        meta.setDisplayName("§e生怪磚");
    ...
    static final public void Composite() {
        ShapedRecipe recipe = new ShapedRecipe(SuperFreedomSurvive.SYSTEM.NamespacedKey.getKey("spawner_block"), Get());
    ...
    static final public void OpenComposite(Player player) {
        ...
        inv.setItem(12, new ItemStack(Material.DRAGON_EGG));
    ...
    static final public void Use(BlockPlaceEvent event) {
        ...
        if (SuperFreedomSurvive.Land.Permissions.Inspection(event.getPlayer(), event.getBlock().getLocation(), "Permissions_BlockPlace")) {
            ...
            // 是否是   ( 存在  數據 )
            if (player_item_tag.getCustomTag(item_key, ItemTagType.STRING).equals("spawner_block")) {
...
*/
                    ...
                    SuperFreedomSurvive.Block.Data.Set(block.getLocation(), "spawner_block");
