...
final public class CommandTpaOk implements CommandExecutor {
    @Override
    final public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 1) {
                    ...
                    if (player_me.getName().equals(args[0])) {
                        ...
                        try {
                            ...
                            if (count > 0) {
                                ...
                                // 更動資料
                                sta.executeUpdate("UPDATE `player-transfer_cache` SET `Result` = '1' WHERE `Player_Tp` = '" + args[0] + "' AND `Time` >= '" + (new Date().getTime() - 1000 * 30) + "' AND `Result` = '0'");
                                ...
                                if (player_to == null) {
                                    ...
                                    // 線上狀態
                                    // 被傳送者 發送訊息
                                    player_to.sendMessage("[傳送系統]  §a" + player_me.getName() + "§f 同意了你的請求");
                                    // 指令使用者 發送訊息
                                    player_me.sendMessage("[傳送系統]  已接受 §a" + args[0]);


                                    // 傳送玩家過去
                                    //Location loc = player_me.getLocation();
                                    // 傳送 player_to 到 player_me
                                    SuperFreedomSurvive.Player.Teleport.Location(
                                            player_to,
                                            player_me.getWorld().getName(),
                                            player_me.getLocation().getBlockX(),
                                            player_me.getLocation().getBlockY(),
                                            player_me.getLocation().getBlockZ()
                                    );
                                    ...
                                    player_to.teleport(
                                            new Location(
                                                    player_me.getWorld(),
                                                    player_me.getLocation().getX(),
                                                    player_me.getLocation().getY(),
                                                    player_me.getLocation().getZ()
                                            )
                                    );
                                    */
                ...
                // ! args[0] 沒有資料
                player_me.sendMessage("[傳送系統]  §c請輸入要 接受傳送 的玩家名稱");
                return false;
            }
        } else {
            // ! 發出命令的人 不是玩家
