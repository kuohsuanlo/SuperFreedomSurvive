    ...
    static final public void Use(Player player, Block block) {
        ...
        if (ok) {
            ...
            if (need != null) {
                ...
                if (amount >= need_amount) {
                    ...
                    if (all.size() > 0) {
                        ...
                        // 放入物品
                        inv.setItem(Integer.parseInt(all.get(random)), need);

                        player.sendMessage("[告示牌系統]  抽獎成功");
                    } else {
                    ...
                    // 所需的物資不夠
                    player.sendMessage("[告示牌系統]  §c所需的抽獎條件物品不夠");
                    return;
                }


            } else {
                player.sendMessage("[告示牌系統]  §c左上第一格請放置抽獎條件物品");
                /*
                if ( all.size() > 0 ) {
                    // 隨機數
                    int random = (int) (Math.random() * all.size());
            ...
            if ( total > 1 ) {
                ...
                org.bukkit.inventory.ItemStack item = inv.getItem( all[ random ] );

                // 給物品
                player.getInventory().addItem( item );

                // 移除箱子物品
                inv.clear( all[ random ] );
            }
            */
        }
    }
